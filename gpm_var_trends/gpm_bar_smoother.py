...
#    _create_initial_covariance_with_gammas,
    _sample_initial_conditions_with_gammas_safe,
    _create_initial_covariance_with_gammas_safe,
...
# In gpm_bar_smoother.py

def fit_gpm_model_with_smoother_with_gamma0(gpm_file_path: str, y: jnp.ndarray,
                               num_warmup: int = 1000, num_samples: int = 2000,
                               num_chains: int = 4, num_extract_draws: int = 100,
                               gamma_init_scaling: float = 0.1, # New parameter for scaling
                               rng_key: jnp.ndarray = random.PRNGKey(0)):
    """
    Fit a GPM-based BVAR model and extract trend/stationary components using simulation smoother.
    UPDATED to use _safe versions of gamma matrix initialization for P0 and initial mean.
    """

    print(f"Parsing GPM file: {gpm_file_path}")

    # Parse GPM file
    parser = GPMParser() # Assuming old parser for now as per Task 5 not being done yet
    gpm_model = parser.parse_file(gpm_file_path)
    ...
    # Create the Numpyro model function
    def gpm_bvar_model(y: jnp.ndarray):
        """Numpyro model based on GPM specification with SAFELY SCALED gamma matrix initialization"""
        T, n_obs = y.shape
        ...
        # Get gamma_list from _sample_var_parameters
        Sigma_u, A_transformed, gamma_list = _sample_var_parameters(gpm_model)

        Sigma_eps = _sample_measurement_covariance(gpm_model) if _has_measurement_error(gpm_model) else None

        # Sample initial conditions using SAFE gamma matrix functions
        init_mean = _sample_initial_conditions_with_gammas_safe(
            ...
            gamma_scaling=gamma_init_scaling # Pass scaling factor
        )
        init_cov = _create_initial_covariance_with_gammas_safe(
            ...
            use_gamma_scaling=gamma_init_scaling, # Pass scaling factor
            fallback_to_original_on_error=True # Allow fallback if gamma matrices are problematic                                        
    ...
    print("Running MCMC...")
    kernel = NUTS(gpm_bvar_model, target_accept_prob=0.9) # Added target_accept_prob
    mcmc = MCMC(kernel,
    ...
    # Check if we have enough posterior samples
    samples = mcmc.get_samples()
    if not samples: # Check if samples dictionary is empty
        print("Warning: No samples obtained from MCMC. Cannot extract components.")
        total_posterior_draws = 0
    else:
        first_param_key = list(samples.keys())[0]
        total_posterior_draws = len(samples[first_param_key])

    actual_num_extract_draws = min(num_extract_draws, total_posterior_draws)

    if num_extract_draws > total_posterior_draws and total_posterior_draws > 0:
        print(f"Warning: Reducing requested simulation smoother draws from {num_extract_draws} to {actual_num_extract_draws} "
              f"as only {total_posterior_draws} posterior samples are available.")
    elif total_posterior_draws == 0:
        print(f"Warning: No posterior samples available. Component extraction will be skipped.")


    trend_draws = jnp.empty((0, y.shape[0], ss_builder.n_trends), dtype=_DEFAULT_DTYPE)
    stationary_draws = jnp.empty((0, y.shape[0], ss_builder.n_stationary), dtype=_DEFAULT_DTYPE)
    trend_hdi = None
    stationary_hdi = None

    if actual_num_extract_draws > 0:
        try:
            trend_draws, stationary_draws = extract_gpm_trends_and_components(
                mcmc, y, gpm_model, ss_builder,
                num_draws=actual_num_extract_draws,
                rng_key=extract_key
            )
            ...
            print("Computing HDI intervals using ArviZ...")
            trend_draws_np = np.asarray(trend_draws)
            stationary_draws_np = np.asarray(stationary_draws)
            ...
            if trend_hdi and stationary_hdi and not (np.any(np.isnan(trend_hdi['low'])) or np.any(np.isnan(stationary_hdi['low']))):
                print("HDI computed successfully using ArviZ!")
            else:
                print("Warning: HDI computation resulted in NaNs or was not performed (e.g. too few draws).")
            ...
            print(f"Error during component extraction: {e}")
            # Ensure empty arrays are returned if extraction fails mid-way
            trend_draws = jnp.empty((0, y.shape[0], ss_builder.n_trends), dtype=_DEFAULT_DTYPE)
            stationary_draws = jnp.empty((0, y.shape[0], ss_builder.n_stationary), dtype=_DEFAULT_DTYPE)
            trend_hdi = None
            stationary_hdi = None
    else:
        print("Skipping component extraction as no posterior draws are available or requested.")
...
def fit_gpm_model_selectable_init(
    ...
    num_warmup: int = 1000, num_samples: int = 2000,
    num_chains: int = 4, num_extract_draws: int = 100,
    use_gamma_initialization: bool = False, # Flag to choose initialization
    gamma_init_scaling: float = 0.1,      # Only used if use_gamma_initialization is True
    rng_key: jnp.ndarray = random.PRNGKey(0)
):
    """
    ...
    if use_gamma_initialization:
        print("Using Gamma-based P0 initialization...")
        ...
        print("Using Standard P0 initialization...")
...
def quick_example():
    ...
    results = complete_gpm_workflow(
        ...
        use_gamma_init=True, # Pass the flag
        gamma_scale_factor=1.0,  # Pass the scaling
        generate_plots=True
    )
