...
#     state_dim: int, n_dynamic_trends: int,
#     gamma_list: List[jnp.ndarray], # Expected: [Gamma_0, Gamma_1, ..., Gamma_{p-1}] (length p)
#     n_stationary: int, var_order: int,
#     gamma_scaling: float = 1.0
# ) -> jnp.ndarray:
#     """
#     Builds the initial state covariance matrix P0 for testing.
...
#     Raises:
#         RuntimeError: If gamma_list is invalid for gamma-based P0, or if matrix construction fails.
#     """
...
#             jnp.eye(n_dynamic_trends, dtype=_DEFAULT_DTYPE) * 1e6 # Diffuse
#         )
...
#         if not gamma_list or len(gamma_list) != var_order: # Expect p elements: Gamma_0 to Gamma_{p-1}
#             raise RuntimeError(f"Build P0 Cov Error: Gamma-based P0 requires 'gamma_list' with length equal to 'var_order' ({var_order}). Got length {len(gamma_list) if gamma_list else 'None'}.")

#         var_block_cov = jnp.zeros((var_state_total_dim, var_state_total_dim), dtype=_DEFAULT_DTYPE)
        
#         for r_idx_block in range(var_order):  # Block row index (0 to p-1)
#             for c_idx_block in range(var_order):  # Block col index (0 to p-1)
#                 lag_diff_h = abs(r_idx_block - c_idx_block) # This is the lag h for Gamma_h

#                 # gamma_list is [Gamma_0, Gamma_1, ..., Gamma_{p-1}]
#                 # So gamma_list[lag_diff_h] should be Gamma_h
#                 if not (0 <= lag_diff_h < len(gamma_list)): # Should always be true given len(gamma_list) == var_order
#                      raise RuntimeError(f"Build P0 Cov Error: Calculated lag difference {lag_diff_h} is out of bounds for gamma_list (len {len(gamma_list)}). This indicates an internal logic error.")

#                 gamma_h = gamma_list[lag_diff_h]
#                 if gamma_h is None or gamma_h.shape != (n_stationary, n_stationary) or not jnp.all(jnp.isfinite(gamma_h)):
#                     raise RuntimeError(f"Build P0 Cov Error: gamma_list[{lag_diff_h}] (Gamma_{lag_diff_h}) is invalid (shape {gamma_h.shape if gamma_h is not None else 'None'}, expected ({n_stationary},{n_stationary}), or contains non-finite values).")

#                 current_block_cov = gamma_h * gamma_scaling
#                 if r_idx_block > c_idx_block: # Lower block diagonal part, use transpose of Gamma_h
#                     current_block_cov = current_block_cov.T
                
#                 row_s, row_e = r_idx_block * n_stationary, (r_idx_block + 1) * n_stationary
#                 col_s, col_e = c_idx_block * n_stationary, (c_idx_block + 1) * n_stationary
#                 var_block_cov = var_block_cov.at[r_s:r_e, c_s:c_e].set(current_block_cov)
        
#         if not (var_start_idx + var_state_total_dim <= state_dim):
#              raise RuntimeError(f"Build P0 Cov Error: VAR block index range [{var_start_idx}:{var_start_idx + var_state_total_dim}] exceeds state_dim {state_dim}.")
...
#     # Finalize: ensure symmetry and positive definiteness
#     init_cov = (init_cov + init_cov.T) / 2.0 + _KF_JITTER * jnp.eye(state_dim, dtype=_DEFAULT_DTYPE)

#     if not jnp.all(jnp.isfinite(init_cov)):
#          raise RuntimeError("Build P0 Cov Error: Final P0 matrix contains non-finite values.")
#     try:
#         jnp.linalg.cholesky(init_cov) # Test for PSD
#     except Exception as e:
#         diag_str = str(jnp.diag(init_cov)[:min(10, state_dim)]) + ("..." if state_dim > 10 else "")
#         raise RuntimeError(f"Build P0 Cov Error: Final P0 matrix (dim {state_dim}x{state_dim}) not positive semi-definite: {e}. Diagonal starts: {diag_str}") from e
#     return init_cov

# def _resolve_parameter_value(param_key_base: str, # e.g., "SHK_TREND1" or "rho"
#                              param_values_input: Dict[str, Any],
#                              estimated_params_gpm: Dict[str, Any], # GPM's estimated_params section
#                              is_shock_std_dev: bool = False
#                              ) -> float:
#     """
#     Resolves a parameter's value:
#     1. Check `param_key_base` in `param_values_input`.
#     2. If `is_shock_std_dev`, check `sigma_{param_key_base}` in `param_values_input`.
#     3. Check `param_key_base` in `estimated_params_gpm` (use prior mode/mean).
#     4. If `is_shock_std_dev`, check `sigma_{param_key_base}` in `estimated_params_gpm`.
#     Raises ValueError if not found or prior is unusable.
#     """
#     val_resolved = None
#     checked_keys = []

#     # Check direct name in param_values_input
#     checked_keys.append(param_key_base)
#     if param_key_base in param_values_input:
#         val_resolved = param_values_input[param_key_base]
#     # If shock, check sigma_ prefixed name in param_values_input
...
#     # If not found in param_values_input, try GPM priors
#     if val_resolved is None:
#         prior_spec_to_use = None
#         # Check direct name in GPM priors
...
#         # If shock, check sigma_ prefixed name in GPM priors
#         elif is_shock_std_dev:
...
#                 mode = beta / (alpha + 1.0) if alpha > -1.0 else beta # Ensure alpha+1 is valid
#                 val_resolved = mode if mode is not None and mode > 0 else 0.01 # Fallback for non-positive mode
#             elif prior_spec_to_use.distribution == 'normal_pdf' and len(prior_spec_to_use.params) >= 1:
#                 mean_val = prior_spec_to_use.params[0]
#                 val_resolved = abs(mean_val) if is_shock_std_dev and mean_val == 0 else mean_val # Ensure positive for std dev if mean is 0
#                 if is_shock_std_dev and val_resolved == 0: val_resolved = 0.01 # Small positive if abs(mean) is 0
#             else:
#                 raise ValueError(f"Parameter '{param_key_base}': Unsupported prior distribution '{prior_spec_to_use.distribution}' or insufficient params in GPM estimated_params.")
    
#     if val_resolved is None:
#         raise ValueError(f"Parameter '{param_key_base}' could not be resolved. Checked: {', '.join(checked_keys)}.")
    
#     try:
#         val_float = float(val_resolved)
#     except (TypeError, ValueError) as e:
#         raise ValueError(f"Parameter '{param_key_base}': Resolved value '{val_resolved}' cannot be converted to float.") from e

#     if is_shock_std_dev and val_float <= 0:
#         raise RuntimeError(f"Parameter '{param_key_base}' (shock std dev): Resolved value must be positive, got {val_float}.")
#     return val_float


# From the refactored gpm_prior_evaluator.py provided previously
