...
def compute_hdi_robust(draws: Union[jnp.ndarray, np.ndarray],
    ...
    if len(draws_np.shape) > 1 and draws_np.shape[1] == 0:
    ...
    try:
        if ARVIZ_AVAILABLE:
            ...
            if draws_np.ndim <= 2:
                # For 1D or 2D arrays, ArviZ can handle directly
                hdi_result = az.hdi(draws_np, hdi_prob=hdi_prob)
                if hdi_result.ndim == 1:
                    return hdi_result[0], hdi_result[1]
                else:
                    return hdi_result[:, 0], hdi_result[:, 1]
            else:
                # For 3D arrays (n_draws, n_timesteps, n_variables), compute per variable
                ...
                for v in range(n_variables):
                    variable_draws = draws_np[:, :, v]  # (n_draws, n_timesteps)
                    hdi_result = az.hdi(variable_draws, hdi_prob=hdi_prob)
                    hdi_lower[:, v] = hdi_result[:, 0] 
                    hdi_upper[:, v] = hdi_result[:, 1]
                
                return hdi_lower, hdi_upper
        
        # Fallback to percentiles (existing code)
        hdi_lower = np.percentile(draws_np, (1 - hdi_prob) / 2 * 100, axis=0)
        hdi_upper = np.percentile(draws_np, (1 + hdi_prob) / 2 * 100, axis=0)
        return hdi_lower, hdi_upper
