...
# Configure JAX
from .jax_config import configure_jax # This line executes the config
...
# # This function creates P0 when gamma_list is available
...
# ) -> jnp.ndarray:
#     """
#     Gamma-based initial covariance for MCMC context.
#     Trends variance: 1e6
#     VAR fallback variance (if gamma_list malformed): 4.0
#     """
#     init_cov = jnp.eye(state_dim, dtype=_DEFAULT_DTYPE)
    
#     # Context: MCMC Sampling
#     trend_var_scale_mcmc = 1e6  
#     var_fallback_scale_mcmc = 4.0 
#     print(f"  P0 (MCMC, Gamma-Based): Using trend variance scale = {trend_var_scale_mcmc}")
...
#     gamma_list_is_valid_mcmc = (
#         gamma_list_draw and
#         len(gamma_list_draw) == var_order and
#         gamma_list_draw[0] is not None and
#         hasattr(gamma_list_draw[0], 'shape') and
#         gamma_list_draw[0].shape == (n_stationary, n_stationary) and
#         all(
#             g is not None and hasattr(g, 'shape') and g.shape == (n_stationary, n_stationary) and jnp.all(jnp.isfinite(g))
#             for g in gamma_list_draw
#         )
#     )

#     if n_stationary > 0 and var_order > 0 and gamma_list_is_valid_mcmc:
#         print(f"  P0 (MCMC, Gamma-Based): Building VAR block using gamma matrices (scaling: {gamma_scaling}).")
...
#                 blk_unscaled = gamma_list_draw[lag_d] # Validity checked by gamma_list_is_valid_mcmc
...
#         else: # Should not happen if dimensions are consistent
#             print(f"    Warning: P0 (MCMC, Gamma-Based) VAR block dimension issue. Applying fallback.")
#             init_cov = init_cov.at[var_start_idx:var_start_idx+var_state_total_dim, 
#                                    var_start_idx:var_start_idx+var_state_total_dim].set(
#                                        jnp.eye(var_state_total_dim, dtype=_DEFAULT_DTYPE) * var_fallback_scale_mcmc
#                                    )

#     elif var_state_total_dim > 0: # VAR part exists but gamma_list was not valid
#         print(f"  P0 (MCMC, Gamma-Based): Gamma list not suitable. Using VAR fallback scale {var_fallback_scale_mcmc}.")
#         init_cov = init_cov.at[var_start_idx:var_start_idx+var_state_total_dim, 
#                               var_start_idx:var_start_idx+var_state_total_dim].set(
#                                   jnp.eye(var_state_total_dim, dtype=_DEFAULT_DTYPE) * var_fallback_scale_mcmc
#                               )
    
#     # More jitter for MCMC stability during sampling
#     init_cov = (init_cov + init_cov.T) / 2.0 + _KF_JITTER * 10 * jnp.eye(state_dim, dtype=_DEFAULT_DTYPE) 
    
#     # Check PSD within model is tricky due to JAX tracing.
#     # It's better to ensure upstream (e.g. gamma_list_draw, scaling) are sensible.
#     # A heavy penalty to loglik is applied if Cholesky of Q fails later, which indirectly handles bad P0.
#     print(f"  P0 (MCMC, Gamma-Based): Constructed. Diag min/max: {jnp.min(jnp.diag(init_cov)):.2e}, {jnp.max(jnp.diag(init_cov)):.2e}")
#     return init_cov


# # This function creates P0 when gamma_list is NOT available or not used
# def _create_initial_covariance_standard(state_dim: int, n_dynamic_trends: int) -> jnp.ndarray:
#     """
#     Standard initial covariance for MCMC context.
#     Trends variance: 1e6
#     VAR fallback variance: 4.0
#     """
#     # Context: MCMC Sampling
#     trend_var_scale_mcmc = 1e6  
#     var_fallback_scale_mcmc = 4.0
#     print(f"  P0 (MCMC, Standard): Using trend variance scale = {trend_var_scale_mcmc}, VAR fallback scale = {var_fallback_scale_mcmc}")

#     init_cov = jnp.eye(state_dim, dtype=_DEFAULT_DTYPE) * trend_var_scale_mcmc
    
#     if state_dim > n_dynamic_trends: # If there are non-trend (i.e., VAR) states
#         init_cov = init_cov.at[n_dynamic_trends:, n_dynamic_trends:].set(
#             jnp.eye(state_dim - n_dynamic_trends, dtype=_DEFAULT_DTYPE) * var_fallback_scale_mcmc
#         )
    
#     # More jitter for MCMC stability
#     init_cov = (init_cov + init_cov.T) / 2.0 + _KF_JITTER * 10 * jnp.eye(state_dim, dtype=_DEFAULT_DTYPE) 
#     print(f"  P0 (MCMC, Standard): Constructed. Diag min/max: {jnp.min(jnp.diag(init_cov)):.2e}, {jnp.max(jnp.diag(init_cov)):.2e}")
#     return init_cov
...
def _create_initial_covariance_gamma_based(
    ...
    trend_var_scale_mcmc = 1e6
    var_fallback_scale_mcmc = 4.0
    # Temporarily comment out the print statement that causes issues during JAX tracing
    # print(f"  P0 (MCMC, Gamma-Based): Using trend variance scale = {trend_var_scale_mcmc}")
    ...
    if n_stationary > 0 and var_order > 0 and gamma_list_is_valid_mcmc:
        ...
        if var_start_idx + var_state_total_dim <= state_dim:
            ...
            # print(f"    Warning: P0 (MCMC, Gamma-Based) VAR block dimension issue. Applying fallback.") # Comment out for tracing
            init_cov = init_cov.at[var_start_idx:var_start_idx+var_state_total_dim, 
                                   var_start_idx:var_start_idx+var_state_total_dim].set(
                                       jnp.eye(var_state_total_dim, dtype=_DEFAULT_DTYPE) * var_fallback_scale_mcmc
                                   )
    elif var_state_total_dim > 0:
        # print(f"  P0 (MCMC, Gamma-Based): Gamma list not suitable. Using VAR fallback scale {var_fallback_scale_mcmc}.") # Comment out for tracing
        init_cov = init_cov.at[var_start_idx:var_start_idx+var_state_total_dim, 
                              var_start_idx:var_start_idx+var_state_total_dim].set(
                                  jnp.eye(var_state_total_dim, dtype=_DEFAULT_DTYPE) * var_fallback_scale_mcmc
                              )
    ...
    # Comment out the problematic print statement:
    # print(f"  P0 (MCMC, Gamma-Based): Constructed. Diag min/max: {jnp.min(jnp.diag(init_cov)):.2e}, {jnp.max(jnp.diag(init_cov)):.2e}")
    return init_cov
...
def _create_initial_covariance_standard(state_dim: int, n_dynamic_trends: int) -> jnp.ndarray:
    ...
    trend_var_scale_mcmc = 1e6  
    var_fallback_scale_mcmc = 4.0
    # Comment out the print statement that causes issues during JAX tracing
    # print(f"  P0 (MCMC, Standard): Using trend variance scale = {trend_var_scale_mcmc}, VAR fallback scale = {var_fallback_scale_mcmc}")
