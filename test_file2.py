...
import xarray # Import xarray to check type (not directly used for HDI anymore, but might be elsewhere)
import time 
...
# Update example_usage to call the new percentile function
# Replace calls to compute_hdi_with_arviz with compute_hdi_with_percentiles
...
def example_usage():
    ...
    if num_extract_draws < num_extract_draws_desired:
         print(f"Reducing requested simulation smoother draws from {num_extract_draws_desired} to {num_extract_draws} as only {total_posterior_draws} posterior samples are available.")
