...
custom_plot_specs_factor_model = [
    {
        ...
        "series_to_plot": [
            {'type': 'observed', 'name': 'r_us', 'label': 'Observed Rshort_US', 'style': '--'}, # Proxy
    ...
    },
    {
        ...
        "series_to_plot": [
            {'type': 'observed', 'name': 'y_us', 'label': 'Observed Y_US', 'style': '-'},
...
custom_plot_specs_factor_model = [
    {
        "title": "Simple Test: US Output",
        "series_to_plot": [
            {'type': 'observed', 'name': 'y_us', 'label': 'Observed Y_US', 'style': '-'},
            # Only include trends that definitely exist
            {'type': 'trend', 'name': 'y_US_trend', 'label': 'US Output Trend', 'show_hdi': True, 'color': 'blue'}
        ]
    },
    {
        "title": "Simple Test: US Inflation", 
        "series_to_plot": [
            {'type': 'observed', 'name': 'pi_us', 'label': 'Observed PI_US', 'style': '-'},
            # Check if this trend exists in your model first
            {'type': 'trend', 'name': 'pi_US_full_trend', 'label': 'US Inflation Trend', 'show_hdi': True, 'color': 'red'}
        ]
    }
]
...
if results_model:
    ...
    print(f"Final check - Number of draws: {results_model.n_draws}")
    print(f"Final check - Log-likelihood: {results_model.log_likelihood}")
    
    # Debug: Print available trend variables to verify your custom plot specs
    print(f"\nAvailable trend variables for custom plots:")
    for i, name in enumerate(results_model.trend_names):
        print(f"  {i:2d}: {name}")
    
    # Check if your custom plot variables exist
    required_trends = ['rr_US_full_trend', 'r_w_trend', 'r_US_dev_trend', 'factor_r_devs', 
                      'r_US_idio_trend', 'pi_US_full_trend', 'pi_w_trend', 'pi_US_dev_trend',
                      'y_US_trend']
    missing_trends = []
    for trend in required_trends:
        if trend not in results_model.trend_names:
            missing_trends.append(trend)
    
    if missing_trends:
        print(f"\nWARNING: Missing trend variables for custom plots:")
        for trend in missing_trends:
            print(f"  - {trend}")
        print(f"\nThis is why your custom plots are not appearing.")
        print(f"Check your GPM file to ensure these trends are defined correctly.")
    else:
        print(f"\n✓ All required trend variables found for custom plots.")
