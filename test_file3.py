...
from numpyro.diagnostics import summary as numpyro_summary # Import the function
...
import time # Import time for timing operations if needed
...
def bvar_model(y: jnp.ndarray, n_lags: int = 2):
    ...
    numpyro.deterministic("Sigma_u", Sigma_u)
    numpyro.deterministic("Sigma_eta", Sigma_eta)
...
def extract_trends_and_components(mcmc, y: jnp.ndarray, n_lags: int = 2,
    ...
    # Now we need A_transformed from the samples
    if 'A_transformed' not in samples:
         print("Error: 'A_transformed' not found in MCMC samples. Was it declared as deterministic in the model?")
         # Return empty arrays with correct shape
         return jnp.empty((0, T, n_vars), dtype=_DEFAULT_DTYPE), jnp.empty((0, T, n_vars), dtype=_DEFAULT_DTYPE)


    n_posterior = len(samples['A_transformed']) # Use the length of A_transformed samples
...
def example_usage():
    ...
    # # 3. Get all samples (including deterministics)
    # samples = mcmc.get_samples() # group_by_chain=False flattens chains if you have multiple

    # # 4. Filter samples for deterministic variables only
    # deterministic_samples = {
    #     name: samples[name]
    #     for name in samples
    #     if name.startswith(("A_transformed", "Sigma_u", "Sigma_eta")) # Filter by your deterministic variable names
    # }

    # # 5. Print summary for deterministic variables
    # print("\nSummary Estimated Parameters:")
    # numpyro_summary(deterministic_samples)
    ...
    total_posterior_draws = mcmc.get_samples()['A_transformed'].shape[0] # Check A_transformed length
    ...
    # Use compute_hdi_with_percentiles instead of compute_hdi_with_arviz
